...
class PPOTrainer:
    ...
    DEFAULT_CFG: Dict[str, Any] = dict(
        ...
        # Architecture
        hidden_channels = 32,
        num_layers      = 2,       # 2 for PoC; 3 for full experiment
        num_ticks       = 3,
        # PPO
        learning_rate   = 3e-4,
        gamma           = 0.97,
        ...
        max_grad_norm   = 10.0,
        ppo_epochs      = 4,
        minibatch_size  = 256,     # samples per gradient update
        rollout_steps   = 20,
        # Training budget
    ...
    def __init__(self, env, cfg: Optional[Dict[str, Any]] = None, device: str = "auto"):
        ...
        self.agent = DRCAgent(
            obs_channels    = self.cfg["obs_shape"][0],
    ...
    def train(self, save_path: Optional[str] = None, log_every: int = 1_000_000):
        """
        Run PPO until target_steps total environment steps.

        Args:
            save_path:  Base path for checkpoints; e.g. "checkpoints/agent".
                        Saves as save_path_{step}.pt every save_every steps.
            log_every:  Print metrics every this many steps.
        """
        cfg = self.cfg
        n_envs = cfg["num_envs"]
        T = cfg["rollout_steps"]
        ...
        while self.global_step < target:
            ...
            for _ in range(T):
                ...
                # Track episode stats before masking hidden state
                ep_rewards += reward
                ...
                # Mask hidden states for finished episodes (reuse done tensor on GPU)
            ...
            # ── Bootstrap value at final step ─────────────────────────────────
            with torch.no_grad():
                obs_t = torch.FloatTensor(obs).to(self.device)
                last_value = self.agent.get_value(obs_t, hidden)
                last_value = last_value.squeeze(-1).cpu().numpy()

            dones_last = np.zeros(n_envs, dtype=bool)  # not done at bootstrap
            self.buffer.compute_returns_and_advantages(last_value, dones_last)
            ...
            if self.global_step - last_log_step >= log_every:
                ...
                mean_rew = np.mean(self._episode_rewards[-200:]) if self._episode_rewards else 0.0
                print(
                    f"Step: {self.global_step/1e6:.1f}M | "
                    f"Solve rate: {solve_rate:.3f} | "
                    f"Mean reward: {mean_rew:.2f} | "
                    f"Entropy: {losses['entropy']:.3f} | "
                    f"VF loss: {losses['value_loss']:.4f} | "
                    f"SPS: {sps:.0f}",
                    flush=True,
                )
    ...
    def _update(self) -> dict:
        ...
        for _ in range(cfg["ppo_epochs"]):
            for batch in self.buffer.get_minibatches(cfg["minibatch_size"]):
                ...
                # Detach hidden states — no BPTT across rollout boundaries
